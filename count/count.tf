resource "aws_instance" "web" {
  ...
  count = length(var.instance_names) # this length(function)
 ...
 resource "aws_route53_record" "www" {
  ...
  zone_id = var.zone_id
  name    = "${var.instance_names[count.index]}.${var.domain_name}" # interploation mixing two variable and static value 
  type    = "A"
  ttl     = 1
